...
string PGaudio::preload(QString path, QString fileName, int voices)
{
    ...
    bufferID = alutCreateBufferFromFile(
            applicationDirectory.append(fileName).toStdString().c_str());
...
string PGaudio::play(QString fileName)
{
    ...
    if (!bufferID)
        return "Could not find the file " + fileName.toStdString()
                + " . Maybe it hasn't been loaded.";
